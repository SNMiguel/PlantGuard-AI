...
Plant disease costs farmers billions each year, and a diagnosis alone doesn't tell them what to *do*. PlantGuard-AI pairs a 95%-accurate vision model with a reasoning agent that runs a grounded **differential diagnosis**, quantifies the **dollars at risk**, and prescribes treatment — every answer cited via Microsoft Foundry IQ.

## What it does
1. A farmer uploads a leaf photo (and field size) in a Gradio web app.
...
5. **Economic exposure:** a deterministic, code-computed `$ at risk` estimate for the field — clearly labeled an estimate, never LLM-fabricated.
